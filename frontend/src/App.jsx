...
function App() {
  ...
  // --- UI SECTIONS EXTRACTED FOR RESPONSIVE REUSE ---
  ...
  return (
    <div className="h-[100dvh] w-full bg-[#000000] flex flex-col overflow-hidden text-gray-300 font-sans">
      ...
      <main className="flex-1 min-h-0 w-full p-2 md:p-4">
        {isMobile ? (
          /* --- MOBILE LAYOUT: Native scrollable column --- */
          <div className="h-full overflow-y-auto custom-scrollbar flex flex-col gap-4 pb-4">
            {/* Editor stays at top with a solid min-height so the keyboard doesn't crush it */}
            <div className="flex-none h-[60vh] min-h-[400px] bg-[#262626] rounded-lg border border-[#3e3e42] flex flex-col overflow-hidden shadow-lg">
              {editorContent}
            </div>
            
            {/* Results block sits below */}
            <div className="flex-none min-h-[400px] bg-[#262626] rounded-lg border border-[#3e3e42] flex flex-col overflow-hidden shadow-lg">
              {resultsContent}
            </div>
          </div>
        ) : (
